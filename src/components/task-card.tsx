...
const TaskCard = ({ remaining, status, amount, request }: { remaining: number; status: string; amount: number; request: string; }) => {
    const getBadgeProps = () => {
        switch (status) {
            case "Doing":
                return { text: "Receiving", color: "text-green-700", border: "border-green-500" };
            case "Audits":
    ...
    return (
        <Card className="w-full shadow-md rounded-2xl p-4 space-y-3">
                    ...
                    <div className="hidden h-full items-center gap-6 lg:flex">
                        {tabs.map((tab) => (
                            <button
                                ...
                                className={`cursor-pointer rounded-md p-2 font-semibold backdrop-blur-lg transition-colors lg:text-lg ${activeTab === tab.id ? "bg-blue-500/20 text-white" : "hover:bg-blue-500/10 text-gray-300"
                                    }`}
                            >
                                {tab.label}
                            </button>
